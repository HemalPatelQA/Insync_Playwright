...
test.describe.serial('Verify Dashboard Functionality', () => {
  test.beforeAll(async ({ browser }) => {
    ...
    console.log('Session loaded and page objects initialized.');
  });
  ...
  //Actual Name : Burton Hills
  test.fixme('Verify LoggedIn Facility with Fixme', async () => {
    await expect(OD.loggedInFacility).toHaveAttribute('title', 'Burton Hill', { timeout: 10000 });
  });

  test.fail('Verify LoggedIn Facility with fail', async () => {
    await expect(OD.loggedInFacility).toHaveAttribute('title', 'Burton Hillsss', { timeout: 10000 });
  });
